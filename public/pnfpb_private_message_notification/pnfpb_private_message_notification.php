		...
		*/
			...
			if (get_option('pnfpb_ic_fcm_bprivatemessage_enable') == 1 && ($apiaccesskey != '' && $apiaccesskey != false) || (get_option('pnfpb_ic_fcm_bprivatemessage_enable') == 1 && (get_option( 'pnfpb_onesignal_push' ) === '1' || get_option('pnfpb_httpv1_push') === '1' || get_option('pnfpb_progressier_push') === '1' || get_option('pnfpb_webtoapp_push') === '1'))) {
				...
				foreach ( $recipients as $recipient ) {
					...
					$notificationtitle = $sender_name.esc_html( __(' sent you private message',"push-notification-for-post-and-buddypress"));
					...
					if ( class_exists( 'Better_Messages' )) {
						...
						if (function_exists('bp_members_get_user_url')) {
							...
							$messageurl = esc_url( bp_members_get_user_url( $recipient->user_id ).$bettermessages_slug);
						
						} else {
						
							$messageurl = esc_url( bp_core_get_user_domain( $recipient->user_id ).$bettermessages_slug);
						}
						
            			$current_user_id = $recipient->user_id;

           				$url_overwritten = apply_filters( 'bp_better_messages_page', null, $current_user_id );
            			...
            			if( $url_overwritten !== null ){
            				...
            				if( is_user_logged_in() || wp_doing_cron() ) {
                				...
                				if (class_exists('AsgarosForum') && Better_Messages()->settings['chatPage'] === 'asgaros-forum') {
                    				...
                    				$link = $asgarosforum->get_link('profile', $current_user_id) . 'messages';
                					...
                					if (class_exists('WooCommerce') && Better_Messages()->settings['chatPage'] === 'woocommerce') {
                    					...
                    					$link = trailingslashit(get_permalink(get_option('woocommerce_myaccount_page_id'))) . $bettermessages_slug;
            							...
            							if( Better_Messages()->settings['chatPage'] !== '0' ) {
                							...
                							$messageurl = get_permalink( Better_Messages()->settings['chatPage'] );
					...
					if (get_option('pnfpb_progressier_push') === '1') {
						...
						$target_userid_array_values=$wpdb->get_col($wpdb->prepare( "SELECT device_id FROM %i WHERE userid = %d AND device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,5,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",$table_name,$recipient->user_id,'%progressier%' ) );
					...
					if (get_option('pnfpb_webtoapp_push') === '1') {
						...
						$target_userid_array_values=$wpdb->get_col($wpdb->prepare( "SELECT device_id FROM %i WHERE userid = %d AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,5,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",$table_name,$recipient->user_id ) );
					...
					if (get_option('pnfpb_onesignal_push') === '1' && get_option('pnfpb_progressier_push') !== '1') {
						...
						if (get_option('pnfpb_ic_fcm_frontend_enable_subscription') === '1') {
							...
							$target_userid_array_values=$wpdb->get_col($wpdb->prepare( "SELECT userid FROM %i WHERE userid = %d AND device_id LIKE %s AND (SUBSTRING(subscription_option,1,1) = '1' OR SUBSTRING(subscription_option,5,1) = '1' OR subscription_option = '' OR subscription_option IS NULL) LIMIT 2000",$table_name,$recipient->user_id,'%onesignal%' ) );
