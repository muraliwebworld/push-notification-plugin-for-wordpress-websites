...
var pnfpb_progressier_userid = parseInt(pnfpb_ajax_object_progressier_push.userid);

var $j = jQuery.noConflict();

$j(document).ready(function() {
	
	var pnfpbprogressierid = parseInt(pnfpb_progressier_userid)
	
	if (pnfpb_progressier_userid > 0) {
		var data = {
		...
		$j.post(pnfpb_ajax_object_progressier_push.ajax_url, data, function(responseajax) {
			
			var response = JSON.parse(responseajax);

			var subscriptionstatus = response.subscriptionstatus;
			
			var progressier_subscription_id = response.message;
			
			if (subscriptionstatus === 'subscribed' && typeof progressier !== 'undefined') {

				progressier.add({
    						id: progressier_subscription_id
 						});
	...
	$j(".group-button").on("click",".leave-group", function() {
		...
		if (currentToken) {
			...
			$j.post(pnfpb_ajax_object_progressier_push.ajax_url, data, function(response) {
			...
			$j( "#pnfpb_group_users_subscribe_dialog_confirm" ).dialog({  
				...
				closeText : '',
			   	...
			   	buttons: [{
					...
					open: function() {
						$j(this).attr('style','font-weight:bold;color:'+pnfpb_ajax_object_progressier_push.subscribe_button_text_color+';background-color:'+pnfpb_ajax_object_progressier_push.subscribe_button_color+';border:0px');
					},
      				click: function() {
						const d = new Date();
						...
						$j.post(pnfpb_ajax_object_progressier_push.ajax_url, data, function(response) {
							...
							$j(".pnfpb-group-unsubscribe-alert-msg").html("<p>"+pnfpb_ajax_object_progressier_push.group_subscribe_dialog_text_confirm+"</p>");
	...
	if( typeof(pnfpb_unsubscribe_notification_group_element) != 'undefined' && pnfpb_unsubscribe_notification_group_element != null ) {
		...
		$j(document).on( "click",".unsubscribe-notification-group", function(e) {
			...
			if(typeof(pnfpb_group_users_unsubscribe_dialog_confirm) === 'undefined' || pnfpb_group_users_unsubscribe_dialog_confirm === null ) {
				...
				$j(this).append( '<div id="pnfpb_group_users_unsubscribe_dialog_confirm" class="pnfpb_group_users_unsubscribe_dialog_confirm" title="Unsubscribe" style="display:none;"><span class="ui-icon ui-icon-alert" style="float:left; margin:12px 12px 20px 0;"></span>'+pnfpb_ajax_object_progressier_push.group_unsubscribe_dialog_text+'</div><div id="pnfpb-group-unsubscribe-dialog" title="Confirmation"><div id="pnfpb-group-unsubscribe-alert-msg" class="pnfpb-group-unsubscribe-alert-msg"></div></div>' );
				
			}
			
			var unsubscribe_button_text = pnfpb_ajax_object_progressier_push.unsubscribe_button_text;
							...
							$j.post(pnfpb_ajax_object_progressier_push.ajax_url, data, function(response) {
			                	...
			                	if (response != 'fail')
			                    {
    	                           if (response != 'duplicate'){
										...
										$j(".pnfpb-group-unsubscribe-alert-msg").html("<p>"+pnfpb_ajax_object_progressier_push.group_unsubscribe_dialog_text_confirm+"</p>");
	...
	if (pnfpb_ajax_object_progressier_push.userid > 0 && typeof(pnfpb_push_notification_frontend_settings_element) != 'undefined' && pnfpb_push_notification_frontend_settings_element != null) {
