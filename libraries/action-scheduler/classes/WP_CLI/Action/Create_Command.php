...
class Create_Command extends \ActionScheduler_WPCLI_Command {
	...
	public function execute() {
		...
		if ( 0 === $action_id ) {
			$e = new \Exception( __( 'Unable to create a scheduled action.', 'push-notification-for-post-and-buddypress' ) );
			$this->print_error( $e );
		}
	...
	protected function print_success( $action_id, $action_type ) {
		\WP_CLI::success(
			sprintf(
				...
				__( '%1$s action (%2$d) scheduled.', 'push-notification-for-post-and-buddypress' ),
				ucfirst( $action_type ),
				$action_id
			)
		);
