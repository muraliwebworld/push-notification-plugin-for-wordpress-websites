...
class Cancel_Command extends \ActionScheduler_WPCLI_Command {
	...
	protected function cancel_single( $hook, $callback_args, $group ) {
		if ( empty( $hook ) ) {
			\WP_CLI::error( __( 'Please specify hook of action to cancel.', 'push-notification-for-post-and-buddypress' ) );
		}
		...
		if ( null === $result ) {
			$e = new \Exception( __( 'Unable to cancel scheduled action: check the logs.', 'push-notification-for-post-and-buddypress' ) );
			$this->print_error( $e, false );
		}
	...
	protected function cancel_all( $hook, $callback_args, $group ) {
		if ( empty( $hook ) && empty( $group ) ) {
			\WP_CLI::error( __( 'Please specify hook and/or group of actions to cancel.', 'push-notification-for-post-and-buddypress' ) );
		}
		 ...
		 */
		\WP_CLI::success( __( 'Request to cancel scheduled actions completed.', 'push-notification-for-post-and-buddypress' ) );
	}
	...
	protected function print_success() {
		\WP_CLI::success( __( 'Scheduled action cancelled.', 'push-notification-for-post-and-buddypress' ) );
	}
	...
	protected function print_error( \Exception $e, $multiple ) {
		\WP_CLI::error(
			sprintf(
				...
				__( 'There was an error cancelling the %1$s: %2$s', 'push-notification-for-post-and-buddypress' ),
				$multiple ? __( 'scheduled actions', 'push-notification-for-post-and-buddypress' ) : __( 'scheduled action', 'push-notification-for-post-and-buddypress' ),
				$e->getMessage()
			)
		);
	}
