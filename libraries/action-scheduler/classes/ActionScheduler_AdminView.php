...
class ActionScheduler_AdminView extends ActionScheduler_AdminView_Deprecated {
	...
	public function register_system_status_tab( array $tabs ) {
		$tabs['action-scheduler'] = __( 'Scheduled Actions', 'push-notification-for-post-and-buddypress' );
		...
		$screen->add_help_tab(
			array(
				...
				'content' =>
					...
					'</p>' .
					'<h3>' . esc_html__( 'Source', 'push-notification-for-post-and-buddypress' ) . '</h3>' .
					'<p>' .
						esc_html__( 'Action Scheduler is currently being loaded from the following location. This can be useful when debugging, or if requested by the support team.', 'push-notification-for-post-and-buddypress' ) .
					'</p>' .
					'<p>' . $as_source_markup . '</p>' .
					'<h3>' . esc_html__( 'WP CLI', 'push-notification-for-post-and-buddypress' ) . '</h3>' .
					'<p>' .
						sprintf(
							/* translators: %1$s is WP CLI command (not translatable) */
							esc_html__( 'WP CLI commands are available: execute %1$s for a list of available commands.', 'push-notification-for-post-and-buddypress' ),
