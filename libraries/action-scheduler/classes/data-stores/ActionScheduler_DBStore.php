...
",
	...
	public function cancel_action( $action_id ) {
		...
		if ( false === $updated ) {
			...
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to cancel this action. It may may have been deleted by another process.', 'push-notification-for-post-and-buddypress' ), $action_id ) );
		}
	...
	protected function claim_actions( $claim_id, $limit, ?DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			$error = empty( $wpdb->last_error )
				? _x( 'unknown', 'database error', 'push-notification-for-post-and-buddypress' )
				: $wpdb->last_error;
			throw new \RuntimeException(
				sprintf(
					/* translators: %s database error. */
					__( 'Unable to claim actions. Database error: %s.', 'push-notification-for-post-and-buddypress' ),
					$error
				)
			);
		}
		...
		if ( $row_updates < count( $action_ids ) ) {
			throw new RuntimeException(
				sprintf(
					...
					__( 'Unable to release actions from claim id %d.', 'push-notification-for-post-and-buddypress' ),
					$claim->get_id()
				)
			);
		}
	...
	public function mark_failure( $action_id ) {
		...
		if ( empty( $updated ) ) {
			/* translators: %s is the action ID */
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s: we were unable to mark this action as having failed. It may may have been deleted by another process.', 'push-notification-for-post-and-buddypress' ), $action_id ) );
		}
	}
