...
if ( !class_exists( 'PNFPB_ICFM_onetime_push_notifications_List' ) ) {
	...
	public function __construct() {
		...
		parent::__construct( [
			'singular' => __( 'PushNotification', "push-notification-for-post-and-buddypress" ), //singular name of the listed records
			'plural'   => __( 'PushNotifications', "push-notification-for-post-and-buddypress" ), //plural name of the listed records
	...
	public function no_items() {
		...
		esc_html_e( "No one time or scheduled push notifications sent", "push-notification-for-post-and-buddypress" );
	...
	function column_scheduled_timestamp ( $item ) {
		...
		$recurring_status = $pnfpb_selected_datetime_ondemand->format('Y-m-d H:i:s').wp_kses_post( __('<br/>(Local time)',"push-notification-for-post-and-buddypress"));
		...
		if (strtolower($item['status']) !== 'draft' && $item['status'] !== 'sent' && $item['scheduled_type'] !== 'onetime' && $item['scheduled_type'] !== NULL ) {
			...
			try {
				...
				if ($item['scheduled_type'] !== 'recurring') {
					...
					$recurring_status = wp_kses_post( __('In Queue, scheduled for <br/>',"push-notification-for-post-and-buddypress")).$pnfpb_selected_datetime_onetime->format('Y-m-d H:i:s').wp_kses_post( __('<br/>(Local time)',"push-notification-for-post-and-buddypress"));
				}
				else {
				...
				if (count($result) > 0  && isset($result[0]['scheduled_date_gmt'])) {
					...
					$recurring_status = $pnfpb_selected_datetime->format('Y-m-d H:i:s').wp_kses_post( __('<br/>(Local time)',"push-notification-for-post-and-buddypress"));
		...
		if ($item['scheduled_type'] === 'recurring') {
			...
			try {
				...
				if (count($result) > 0 && isset($result[0]['scheduled_date_gmt'])) {
					...
					$recurring_status = esc_html( __('Scheduled.<br/>Next run is on ',"push-notification-for-post-and-buddypress")).$pnfpb_selected_datetime->format('Y/m/d H:i:s').esc_html( __('<br/>(Local time)',"push-notification-for-post-and-buddypress")).$item['status'];					
					...
					if (count($result) > 0  && isset($result[0]['scheduled_date_gmt'])) {
						...
						$recurring_status = wp_kses_post( __('Finished on <br/>',"push-notification-for-post-and-buddypress")).$pnfpb_selected_datetime->format('Y-m-d H:i:s');
					...
					if (count($result) > 0  && isset($result[0]['scheduled_date_gmt'])) {
						...
						$recurring_status = wp_kses_post( __('Failed on <br/>',"push-notification-for-post-and-buddypress")).$pnfpb_selected_datetime->format('Y-m-d H:i:s');
						...
						if (count($result) > 0  && isset($result[0]['scheduled_date_gmt'])) {
							...
							$recurring_status = wp_kses_post( __('In Queue - scheduled on <br/>',"push-notification-for-post-and-buddypress")).$pnfpb_selected_datetime->format('Y-m-d H:i:s');
	...
	function column_action( $item ) {
		...
		$duplicate_nonce = wp_create_nonce( 'pnfpb_duplicate_pushnotification' );
		$cancel_nonce = wp_create_nonce( 'pnfpb_cancel_pushnotification' );
		
		if ($item['action_scheduler_id'] != NULL) {
			$actions = [
				'edit' => sprintf('<a href="?page=%s&status=%s&action=%s&pushnotificationid=%s&_wpnonce=%s">%s</a>','pnfpb_icfmtest_notification',$item['status'],'edit', absint( $item['id'] ),$edit_nonce, __('Edit/Resend', "push-notification-for-post-and-buddypress")),
				'delete' => sprintf( '<a href="?page=%s&action=%s&pushnotificationid=%s&_wpnonce=%s&orderby=id&order=desc">%s</a><br /><br />','pnfpb_icfm_onetime_notifications_list', 'delete', absint( $item['id'] ), $delete_nonce,__('Delete', "push-notification-for-post-and-buddypress") ),
				'duplicate' => sprintf( '<a href="?page=%s&action=%s&pushnotificationid=%s&_wpnonce=%s&orderby=id&order=desc">%s</a><br /><br />','pnfpb_icfm_onetime_notifications_list', 'duplicate', absint( $item['id'] ),$duplicate_nonce,__('Duplicate', "push-notification-for-post-and-buddypress") ),
				'cancel' => sprintf( '<a href="?page=pnfpb_icfm_action_scheduler&status=pending&action=-1&action2=-1&_wpnonce=%s&s=%d&paged=1">%s</a>',$cancel_nonce,absint( $item['action_scheduler_id'] ) ,__('More actions...', "push-notification-for-post-and-buddypress")	)		
			];
		} else {
			$actions = [
				'edit' => sprintf('<a href="?page=%s&status=%s&action=%s&pushnotificationid=%s&_wpnonce=%s">%s</a>','pnfpb_icfmtest_notification',$item['status'],'edit', absint( $item['id'] ),$edit_nonce, __('Edit/Resend', "push-notification-for-post-and-buddypress")),
				'delete' => sprintf( '<a href="?page=%s&action=%s&pushnotificationid=%s&_wpnonce=%s&orderby=id&order=desc">%s</a>','pnfpb_icfm_onetime_notifications_list', 'delete', absint( $item['id'] ), $delete_nonce,__('Delete', "push-notification-for-post-and-buddypress") ),
				'duplicate' => sprintf( '<a href="?page=%s&action=%s&pushnotificationid=%s&_wpnonce=%s&orderby=id&order=desc">%s</a><br /><br />','pnfpb_icfm_onetime_notifications_list', 'duplicate', absint( $item['id'] ),$duplicate_nonce,__('Duplicate', "push-notification-for-post-and-buddypress") )
			];			
		}
	...
	function column_name( $item ) {
		...
		$actions = [
			'edit' => sprintf('<a href="?page=%s&title=%s&content=%s&image_url=%s&click_url=%s&orderby=id&_wpnonce=%s&order=desc">%s</a>','pnfpb_icfm_onetime_notifications_list',$item['title'],$item['content'],$item['image_url'],$item['click_url'],$edit_nonce, __('Edit', "push-notification-for-post-and-buddypress")),
	...
	function get_columns() {
		$columns = [
			...
			'title' => __( 'Title', "push-notification-for-post-and-buddypress" ),
			'content'    => __( 'Content', "push-notification-for-post-and-buddypress" ),
			'image_url' => __('Image url',"push-notification-for-post-and-buddypress"),
			'click_url' => __('Click url',"push-notification-for-post-and-buddypress"),
			'scheduled_timestamp' => __('Current run<br/>date & time',"push-notification-for-post-and-buddypress"),
			'scheduled_type' => __('Schedule type',"push-notification-for-post-and-buddypress"),
			'status' => __('Status',"push-notification-for-post-and-buddypress"),
			'action' => __('Action',"push-notification-for-post-and-buddypress")
		];
	...
	public function prepare_items($search='') {
			...
			if ( isset($_REQUEST['s']) && !isset($_REQUEST['scheduled_type']) ) {
				$this->items = self::get_pushnotifications( $per_page, $current_page, sanitize_text_field(wp_unslash($_REQUEST['s'])),'' );
				$total_items_search = self::get_pushnotifications( 0, $current_page, sanitize_text_field(wp_unslash($_REQUEST['s'])), '' );
			...
			else 
			{
				if ( isset($_REQUEST['scheduled_type']) ) {
					if (isset($_REQUEST['s'])) {
						$this->items = self::get_pushnotifications( $per_page, $current_page, sanitize_text_field(wp_unslash($_REQUEST['s'])),sanitize_text_field(wp_unslash($_REQUEST['scheduled_type'])) );
						$total_items_search = self::get_pushnotifications( 0, $current_page, sanitize_text_field(wp_unslash($_REQUEST['s'])),sanitize_text_field(wp_unslash($_REQUEST['scheduled_type'])) );
						$this->set_pagination_args( [
							'total_items' => count($total_items_search), //WE have to calculate the total number of items
							'per_page'    => $per_page //WE have to determine how many items to show on a page
						] );
					}	else {
						$this->items = self::get_pushnotifications( $per_page, $current_page, '',sanitize_text_field(wp_unslash($_REQUEST['scheduled_type'])) );
						$total_items_search = self::get_pushnotifications( 0, $current_page, '', sanitize_text_field(wp_unslash($_REQUEST['scheduled_type'])) );
						$this->set_pagination_args( [
							'total_items' => count($total_items_search), //WE have to calculate the total number of items
							'per_page'    => $per_page //WE have to determine how many items to show on a page
						] );					
					...
					if ( isset($_REQUEST['scheduled_status']) ) {
						$this->items = self::get_pushnotifications( $per_page, $current_page, '','',sanitize_text_field(wp_unslash($_REQUEST['scheduled_status'])) );
						$total_items_search = self::get_pushnotifications( 0, $current_page, '', '',sanitize_text_field(wp_unslash($_REQUEST['scheduled_status'])) );
						$this->set_pagination_args( [
							'total_items' => count($total_items_search), //WE have to calculate the total number of items
							'per_page'    => $per_page //WE have to determine how many items to show on a page
						] );					
	...
	public function pnfpb_url_scheme( string $url, string $scheme, $orig_scheme ) {
    	if ( ! empty( $url ) && mb_strpos( $url, '?page=pnfpb_icfm_push_notifications_list' ) !== false && isset( $_REQUEST['s'] ) ) {
			...
			$search_nonce = wp_create_nonce( 'pnfpb_search_pushnotification' );
			
        	$url = add_query_arg( 's', urlencode(sanitize_text_field(wp_unslash( $_REQUEST['s'] ))), $url );
	...
	public function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'pnfpb_delete_pushnotification' ) ) {
				...
				$push_notification_id = sanitize_text_field(wp_unslash($_REQUEST['pushnotificationid']));
						
				$notifications = $wpdb->get_results("SELECT * FROM {$notification_table_name} WHERE `id` = {$push_notification_id} ");
				...
				if (count($notifications) > 0) {
					...
					self::delete_pushnotification( absint( $push_notification_id ),$onetime_push_action_scheduler_id,$onetime_push_timestamp,$onetime_push_status );
				}
			}
		...
		if ( 'duplicate' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'pnfpb_duplicate_pushnotification' ) ) {
				...
				if (count($notifications) > 0) {
					...
					$data = array('userid' => $bpuserid,
								...
								'recurring_day_name'=> $onetime_recurring_day_name,							  	
							 	'status'	=> __('Draft', "push-notification-for-post-and-buddypress")
							 );
